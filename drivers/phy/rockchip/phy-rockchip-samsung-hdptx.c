				       ...
				       int submode)
{
	...
	if (mode != PHY_MODE_DP || submode != PHY_SUBMODE_EDP) {
		dev_err(hdptx->dev, "only eDP mode is supported currently\n");
		return -EINVAL;
	}
				       ...
				       struct phy_configure_opts_dp *dp)
{
	...
	// TODO: move to a separate set_lanes?
	regmap_update_bits(hdptx->regmap, 0x081c, LANE_EN,
...
{
	struct rockchip_hdptx_phy *hdptx = dev_get_priv(phy->dev);
	struct phy_configure_opts_dp *opts = params;
	int ret;

	ret = rockchip_hdptx_phy_verify_config(hdptx, opts);
	if (ret) {
	...
	if (opts->set_rate) {
		ret = rockchip_hdptx_phy_set_rate(hdptx, opts);
		if (ret) {
	...
	if (opts->set_voltages) {
		ret = rockchip_hdptx_phy_set_voltages(hdptx, opts);
		if (ret) {
